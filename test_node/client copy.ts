...
socket.on('newPos', (playerData) => {
    ...
    if (color !== game.player.color){
        if (!Others.ids.includes(color)){new Others(color, pos, chunk); console.log('Other created.');}
        else {Others.instances[color].changePos(pos, chunk);}
    }
});
    ...
    drawCentered(){
        ...
        for (let i = 0; i < this.chunkSize; i++){
            for (let j = 0; j < this.chunkSize; j++){
                ...
                try{
                draw(this.tilesProperty[this.tiles[
                    Math.floor((absPos[1]+j)/this.chunkSize)][Math.floor((absPos[0]+i)/this.chunkSize)][(absPos[0]+j)%this.chunkSize][(absPos[0]+i)%this.chunkSize]].color, 
                    j*this.tileSize, i*this.tileSize, this.tileSize);
                } catch(TypeError){
                    console.error(`x:${i}, y:${j}, erreur de dessin.`)
                    console.log(this.tiles);
                }
...
class Others{
    ...
    constructor(color, pos, chunk){
        ...
        console.log('other: ',this.color = color,
            this.pos = pos,
            this.chunk = chunk);
...
class Player{
    ...
    findNextPos(pos, deltaPos, chunk){
        ...
        if (nextPos[0] >= game.board.chunkSize){nextPos[0] = 0; nextChunk[0]++;}
        else if (nextPos[0] <= -1){nextPos[0] = game.board.chunkSize - 1; nextChunk[0]--;}
        if (nextPos[1] >= game.board.chunkSize){nextPos[1] = 0; nextChunk[1]++; }
        else if (nextPos[1] <= -1){nextPos[1] = game.board.chunkSize - 1; nextChunk[1]--;}
        if (nextChunk[0] <= -1){nextChunk[0]=0;}else if (nextChunk[0] >= game.board.chunkGenerationLimit){nextChunk[0]=game.board.chunkGenerationLimit-1;}
        if (nextChunk[1] <= -1){nextChunk[1]=0;}else if (nextChunk[1] >= game.board.chunkGenerationLimit){nextChunk[1]=game.board.chunkGenerationLimit-1;}
    ...
    move(timeStamp){
        ...
        if (newPos){socket.emit('newPos', {color :this.color, pos: this.pos,chunk: this.chunk});}
    }
...
class Bear {
    ...
    constructor(pos, chunk, timeOfLastMove){
        ...
        if (Bear.closestToPlayer === null){Bear.closestToPlayer = this}
    }
    ...
    distancePlayerFind(entity){
        ...
        if (this.distancePlayer < Bear.closestToPlayer.distancePlayer){Bear.closestToPlayer = this;}
    }
    ...
    // goTo(entity){
    //     if (Math.abs(this.distanceX(entity)) > Math.abs(this.distanceY(entity))){return [sign(this.distanceX(entity)), 0];}
    //     return [0, sign(this.distanceY(entity))]
    // }
...
class InputController {
    ...
    updatePosition(){
        let x = 0;
        let y = 0;
        if (this.keyPressed.left) x -= 1;
        if (this.keyPressed.up) y -= 1;
        if (this.keyPressed.right) x += 1;
        if (this.keyPressed.down) y += 1;
        return [x, y]
    }
}