...
export default class Matrix4x4{
    ...
    static multiplyVector(M: Matrix4x4, v: Vector3D){
    ...
    static rotation(X: Radian,Y: Radian,Z: Radian){
        let M = new Matrix4x4();
    ...
    static rotationX(angleRad: Radian){
		let M = new Matrix4x4();
		M.m[0][0] = 1.0;
    ...
    static rotationY(angleRad: Radian){
		let M = new Matrix4x4();
    ...
    static rotationZ(angleRad: Radian)
	{
		let M = new Matrix4x4();
		M.m[0][0] = Math.cos(angleRad);
    ...
    static makeProjection(FOVdegrees: number, aspectRatio: number, near: number, far: number){
    ...
    static multiplyMatrix(M1: Matrix4x4, M2: Matrix4x4){
        let M = new Matrix4x4();
        for(let r=0; r<4; r++){
            for(let c=0; c<4; c++){
    ...
    static pointAt(pos: Vector3D, target: Vector3D, up: Vector3D){
    ...
    static quickInverse(M1: Matrix4x4){ //only for translation / rotation Matrix
